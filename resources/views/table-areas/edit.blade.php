<x-app-layout>
    <x-slot name="title">Edit Area Meja</x-slot>
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90">Edit Area Meja</h2>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ $tableArea->name }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="mx-auto max-w-2xl">
        <x-ui.card title="Informasi Area" description="Perbarui data area meja di bawah ini">
            <form action="{{ route('table-areas.update', $tableArea) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <x-ui.input 
                        ...
                        :value="$tableArea->name"
                        required 
                        autofocus 
                    />

                    <!-- Status -->
                    <x-ui.toggle name="is_active" :checked="$tableArea->is_active" label="Area aktif dan ditampilkan" />
                </div>
